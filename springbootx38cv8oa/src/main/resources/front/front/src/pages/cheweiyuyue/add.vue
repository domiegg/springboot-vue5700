...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车牌号" v-if="type!='cross' || (type=='cross' && !ro.chepaihao)" prop="chepaihao">
            <file-upload
            tip="点击上传车牌号"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.chepaihao?ruleForm.chepaihao:''"
            @change="chepaihaoUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="车牌号" prop="chepaihao">
                <img v-if="ruleForm.chepaihao.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.chepaihao.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.chepaihao.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="车辆状态" prop="cheliangzhuangtai">
            <el-select v-model="ruleForm.cheliangzhuangtai" placeholder="请选择车辆状态" :disabled=" false  ||ro.cheliangzhuangtai" >
              <el-option
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='chepaihao'){
              this.ruleForm.chepaihao = obj[o].split(",")[0];
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`cheweiyuyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('cheweiyuyue/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`cheweiyuyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      chepaihaoUploadChange(fileUrls) {
          this.ruleForm.chepaihao = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
