...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车位编号" prop="cheweibianhao">
              <el-input v-model="ruleForm.cheweibianhao" placeholder="车位编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="校门信息" prop="cheweileixing">
            <el-select v-model="ruleForm.cheweileixing" placeholder="请选择校门信息" :disabled=" false  ||ro.cheweileixing" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="车位状态" prop="cheweizhuangtai">
            <el-select v-model="ruleForm.cheweizhuangtai" placeholder="请选择车位状态" :disabled=" false  ||ro.cheweizhuangtai" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="车位简介" prop="cheweijianjie">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/cheweileixing/cheweileixing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('cheweixinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`cheweixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
