...
<div class="content" :style='{"minHeight":"100vh","padding":"20px 30px 20px","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20230929/29af140d1a664450a9890decb66d8af8.png) no-repeat center top / cover","display":"flex","fontSize":"14px","height":"auto"}'>
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 30px 20px 0px","alignItems":"center","color":"#1e3c4f","flexWrap":"wrap","background":"none","display":"flex","width":"400px","fontSize":"14px","justifyContent":"center","order":"0"}'>
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"12px 0","margin":"0 0 20px 0","borderColor":"rgba(126, 96, 16, .1)","borderRadius":"8px","alignItems":"center","textAlign":"left","background":"#fff","borderWidth":"0 0 0px","display":"flex","width":"100%","borderStyle":"solid"}' v-if="isAuth('cheliangchuchang','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"column-reverse","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"34px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{cheliangchuchangCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>车辆离校总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type3" :style='{"alignContent":"flex-start","padding":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"420px","order":"10"}'>
		<div id="cheliangchuchangChart1" class="echarts1" v-if="isAuth('cheliangchuchang','首页统计')"></div>
		<div id="cheliangchuchangChart2" class="echarts2" v-if="isAuth('cheliangchuchang','首页统计')"></div>
		<div id="cheliangchuchangChart3" class="echarts3" v-if="isAuth('cheliangchuchang','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getcheliangchuchangCount();
		this.cheliangchuchangChat1();
		this.cheliangchuchangChat2();
		this.cheliangchuchangChat3();
	},
	methods:{
		// 词云
		...
		getcheliangchuchangCount() {
			this.$http({
				url: `cheliangchuchang/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.cheliangchuchangCount = data.data
				}
			})
		},
		cheliangchuchangChat1() {
			this.$nextTick(()=>{

        var cheliangchuchangChart1 = echarts.init(document.getElementById("cheliangchuchangChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                cheliangchuchangChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    cheliangchuchangChart1.resize();
                };
            }
        });
      })
    ...
    cheliangchuchangChat2() {
      this.$nextTick(()=>{

        var cheliangchuchangChart2 = echarts.init(document.getElementById("cheliangchuchangChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                cheliangchuchangChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    cheliangchuchangChart2.resize();
                };
            }
        });
      })
    ...
    cheliangchuchangChat3() {
      this.$nextTick(()=>{

        var cheliangchuchangChart3 = echarts.init(document.getElementById("cheliangchuchangChart3"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                cheliangchuchangChart3.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    cheliangchuchangChart3.resize();
                };
            }
        });
      })
